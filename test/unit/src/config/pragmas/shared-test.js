...
test(`@shared population: defaults to enabled (without @shared)`, t => {
  ...
  t.ok(shared.shared.includes(httpLambda), `Got shared lambda: ${httpLambda}`)
  t.ok(shared.shared.includes(eventLambda), `Got shared lambda: ${eventLambda}`)
...
src foo/bar`)
  ...
  t.ok(shared.shared.includes(httpLambda), `Got shared lambda: ${httpLambda}`)
  t.ok(shared.shared.includes(eventLambda), `Got shared lambda: ${eventLambda}`)
  t.notOk(shared.shared.includes(queueLambda), `Did not get lambda: ${queueLambda}`)
  t.ok(fn1.config.shared, `Shared setting enabled in lambda: ${httpLambda}`)
  t.ok(fn2.config.shared, `Shared setting enabled in lambda: ${eventLambda}`)
