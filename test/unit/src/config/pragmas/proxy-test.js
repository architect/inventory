...
let populateProxy = require('../../../../../src/config/pragmas/proxy')
...
test('Set up env', t => {
  t.plan(1)
  t.assert.ok(populateProxy, '@proxy populator is present')
})

test('No @http + no @proxy returns null @proxy', t => {
  t.plan(1)
  let inventory = inventoryDefaults()
  t.assert.equal(populateProxy({ arc: {}, inventory }), null, 'Returned null')
})
...
`)
  ...
  t.assert.equal(proxy.testing, testing, `Got back testing env: ${testing}`)
  t.assert.equal(proxy.staging, staging, `Got back staging env: ${staging}`)
  t.assert.equal(proxy.production, production, `Got back production env: ${production}`)
})
...
`)
  ...
  t.assert.equal(proxy.testing, 'http://foo', `Got back testing env: ${testing}`)
  t.assert.equal(proxy.staging, 'http://bar', `Got back staging env: ${staging}`)
  t.assert.equal(proxy.production, 'http://baz', `Got back production env: ${production}`)
})
...
test('@proxy: validation errors', t => {
  ...
  t.assert.ok(errors.length, '@proxy without @http errored')
...
${envs[2]} foo`)
  ...
  t.assert.ok(errors.length, `@proxy errors when ${envs[0]} isn't present`)
...
${envs[2]} foo`)
  errors = []
  populateProxy({ arc, inventory, errors })
  t.assert.ok(errors.length, `@proxy errors when ${envs[1]} isn't present`)
...
${envs[1]} foo`)
  errors = []
  populateProxy({ arc, inventory, errors })
  t.assert.ok(errors.length, `@proxy errors when ${envs[2]} isn't present`)
...
${envs[2]} foo`)
  errors = []
  populateProxy({ arc, inventory, errors })
  t.assert.ok(errors.length, `@proxy errors with invalid setting`)
