...
let setEnvPlugins = require('../../../../../../src/config/project/plugins/env')
...
test('Set up env', t => {
  t.plan(1)
  t.assert.ok(setEnvPlugins, 'Env plugin setter module is present')
})
...
test('Do nothing if no env setter plugins are present', t => {
  ...
  t.assert.deepEqual(plugins, nulls, 'Plugins remain null')
...
test('Env setter plugin runs', t => {
  ...
  function check (actual, expected, customEnv) {
    t.diagnostic(`Got env:`, actual)
    t.assert.deepEqual(actual.testing, customEnv ? expected.testing : expected, 'Testing env got correct env vars')
    t.assert.deepEqual(actual.staging, customEnv ? expected.staging : expected, 'Staging env got correct env vars')
    t.assert.deepEqual(actual.production, customEnv ? expected.production : expected, 'Production env got correct env vars')
  }
  ...
  pluginOne = function (params) {
    ...
    t.assert.deepEqual(params, { arc: inventory._project.arc, inventory: { inv: inventory } }, 'Inventory is partial, containing the current default inventory + partially built project')
    return varStr
  }
...
test('Env setter plugin errors', t => {
  ...
  t.assert.match(errors[0], /must return an Object/, 'Got correct error')
  ...
  setEnvPlugins({ inventory, errors }, emptyProj)
  t.assert.equal(errors.length, 1, 'Returned an error')
  t.assert.match(errors[0], /is invalid, must be/, 'Got correct error')
  ...
  setEnvPlugins({ inventory, errors }, emptyProj)
  t.assert.equal(errors.length, 1, 'Returned an error')
  t.assert.match(errors[0], /lolidk/, 'Got correct error')
  ...
  setEnvPlugins({ inventory, errors }, emptyProj)
  t.assert.equal(errors.length, 1, 'Returned an error')
  t.assert.match(errors[0], /Runtime plugin/, 'Got correct error')
  ...
  setEnvPlugins({ inventory, errors }, emptyProj)
  t.assert.equal(errors.length, 1, 'Returned an error')
  t.assert.match(errors[0], /already registered/, 'Got correct error')
